...
public class AnalisisPemasukanController {
    ...
    public void initialize() {
        try {
            ...
            System.err.println("AnalisisPemasukanController: SharedDataManager or TransaksiManager not initialized: " + e.getMessage());
            e.printStackTrace();
            return;
        }
    ...
    public void switchToPengeluaranAnalisis(ActionEvent event) {
        try {
            ...
            URL fxmlLocation = getClass().getResource("/pbo/transaksikeuangan/LaporanPengeluaran.fxml");
            System.out.println("DEBUG: FXML Location for LaporanPengeluaran: " + fxmlLocation);
            if (fxmlLocation == null) {
                System.err.println("ERROR: LaporanPengeluaran.fxml not found at specified path!");
                showAlert("Error Pemuatan FXML", "File LaporanPengeluaran.fxml tidak ditemukan. Periksa path dan struktur proyek.");
                return; // Hentikan eksekusi jika FXML tidak ditemukan
            }
            // --- AKHIR PENAMBAHAN UNTUK DEBUGGING ---

            FXMLLoader loader = new FXMLLoader(fxmlLocation); // Gunakan URL yang sudah diverifikasi
